...
export default class EvaluationPage extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.props.params.id && this.props.params.eval_id) {
      ...
      Project.getAllEval(this.props.user.id).then(response => {
        this.setState({evaluationProjectsData: response.projects});
      });
    }
  }
  ...
  render() {
        ...
        <div>
          { this.state.evaluationProjectsData.map((project) => {
            if (project.length > 0) {
              return (
                <div>
                  <PageSubHeading subHeading={true} label={project[0].name}/>
                  <Table className={"responsiveTable:30em"}>
                    ...
                    <Tbody>
                    { project.map((projectEval) => {//Project loop
                      {
                        return projectEval.methodology_project_evaluation.map((evaluation) => { //Evals loop
                          if (evaluation.type === "Doporučená metodika") {
                            return <Tr>
                              ...
                              <Td> <Moment format="HH:mm, DD.MM. YYYY">{projectEval.updated_at}</Moment></Td>
                              <Td>
                                  <Link className="btn btn-default btn-sm"
    ...
    return (
      <div className="Layout-section">
        ...
        <PageSubHeading label="Vyhodnocení projektů"/>
